...
class 模擬抽寶石海星: 模擬測試<噴火龍玩家> {
    override var 目前模型: 模擬測試<噴火龍玩家> {
        模擬抽寶石海星()
    }
    
    override func 是否遊戲結束() -> Bool {
        return 玩家.棄牌堆.有({$0 == .火恐龍})
    }
    
    override func 測試前設置(_ prototypeModel: 模擬測試<噴火龍玩家>, _ 測試次數: Int, _ x: Int, _ 出牌策略: [寶可夢出牌策略]) {
        統計表名稱 = "寶石海星"
...
class 模擬抽噴火龍: 模擬測試<沙奈朵玩家> {
    override var 目前模型: 模擬測試<沙奈朵玩家> {
        模擬抽沙奈朵()
    }
    
    override func 是否遊戲結束() -> Bool {
        return 玩家.棄牌堆.有({$0 == .噴火龍EX})
    }
}
...
class 模擬測試<模擬玩家: 寶可夢玩家>: 寶可夢TCG控制器 {
    ...
    func 設定基礎寶可夢數量範圍(_ 範圍: ClosedRange<Int>) {
        基礎寶可夢數量範圍 = 範圍
    }
    ...
    func loop(_ 測試次數: Int) -> [回合統計表] {
        ...
        let 統計表數量 = 基礎寶可夢數量範圍.count * 所有出牌策略.count
        let 模型: [模擬測試<模擬玩家>] = (0 ..< 統計表數量).map({_ in self.目前模型})
        
        for (出牌策略編號, 出牌策略) in 所有出牌策略.enumerated() {
            for 雜牌基礎寶可夢數量 in 基礎寶可夢數量範圍 {
                let 模型編號 = (雜牌基礎寶可夢數量 - 基礎寶可夢數量範圍.lowerBound) + (出牌策略編號 * 基礎寶可夢數量範圍.count)
